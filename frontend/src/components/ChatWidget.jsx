...
- Time: ${scheduleFormData.timeSlot}`,
  ...
  // Add this helper function to fetch the latest message pair for the session
  async function fetchLatestMessagePair(sessionId) {
    try {
      const res = await fetch(
        `http://localhost:8000/messages/latest?session_id=${sessionId}`
      );
      if (res.ok) {
  ...
  // Add this function to handle chatbot close and send analytics
  async function handleCloseChatbot() {
    const userId = localStorage.getItem("healthcare_user_id");
    const sessionId = localStorage.getItem("healthcare_session_id");
    const latest = await fetchLatestMessagePair(sessionId);
    const payload = {
      user_id: userId,
      session_id: sessionId,
      closed_at: new Date().toISOString(),
      time_spent_seconds: 0, // You can calculate this if you track session start
      last_user_message: latest.user_message,
      last_bot_message: latest.bot_message,
    };
    await fetch("http://localhost:8000/analytics/chatbot_close", {
      method: "POST",
